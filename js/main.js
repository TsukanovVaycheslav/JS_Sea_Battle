  ...
  const game = {
    ...
    generateShip() {
      for (let i = 0; i < this.optionShip.count.length; i++) {
          for (let j = 0; j < this.optionShip.count[i]; j++) {
            ...
            const ship = this.ganerateOptionsShip(size);
    ...
    ganerateOptionsShip(shipSize) {
      ...
      if (this.checkCollision(ship.location)) {
        return this.ganerateOptionsShip(shipSize);
      }
    ...
    // Проверка на столкновения кораблей
    checkCollision(location) {
      for (const coord of location) {
        if (this.collision.has(coord)) { // includes
          return true;
        }
      }
    },
